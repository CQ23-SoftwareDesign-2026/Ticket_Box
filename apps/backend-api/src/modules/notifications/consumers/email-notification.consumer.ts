...
export class EmailNotificationConsumer implements OnModuleInit {
  ...
  private async sendTicketEmail(orderId: string) {
    ...
    const tickets = order.tickets
      .map(
        (ticket, index) =>
          `<li style="margin-bottom:16px"><strong>${ticket.category.name}</strong> — Cổng ${ticket.category.gate_number ?? "Tự do"}<br>Mã vé: <code>${ticket.qr_code_hash}</code><br>QR: <strong>ticket-${index + 1}.png</strong></li>`,
      )
      .join("");
    const sent = await this.email.sendMail({
      to: order.user.email,
      subject: `Xác nhận mua vé ${order.concert.name} - TicketBox`,
      html: `<h1>Mua vé thành công</h1><p>Chào ${order.user.full_name}, e-ticket QR của bạn được đính kèm trong email này:</p><ul>${tickets}</ul><p>Vui lòng bảo mật QR và xuất trình ảnh tương ứng khi check-in.</p>`,
