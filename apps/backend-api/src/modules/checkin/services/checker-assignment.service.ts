...
export class CheckerAssignmentService {
    ...
    async createAssignment(dto: CreateCheckerAssignmentDto) {
        ...
        const isChecker = checkerUser.user_roles.some((ur) => ur.role.name === 'Checker');
        if (!isChecker) {
            throw new BadRequestException(`User ${dto.checker_id} does not have the 'Checker' role`);
        }
        ...
        if (!validGates.includes(dto.gate_number)) {
            throw new BadRequestException(`Gate number ${dto.gate_number} is not valid for concert ${dto.concert_id}`);
        }
        ...
        if (existing) {
            throw new BadRequestException('Checker is already assigned to this concert');
        }
        ...
        if (existingGate) {
            throw new BadRequestException(`Gate number ${dto.gate_number} is already assigned to another checker`);
        }
    ...
    async getAvailableGates(concertId: string) {
        ...
        const assignedGates = concert.checker_assignments.map((ca) => ca.gate_number);
